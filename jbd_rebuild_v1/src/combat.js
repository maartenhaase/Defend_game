(() => {
  ...
  function weapon(s,kind){ return (s.profile?.weapons&&s.profile.weapons[kind]) || C.weapons[kind]; }
  ...
  function fireCharge(s,hold,x,y){
    ...
    if(hold<C.input.apCharge){
      const W=weapon(s,'mg');
      for(let i=0;i<W.burst;i++) s.pendingShots.push({t:i*W.cadence,kind:'mg',a:base,x,y,index:i});
  ...
  function spawnProjectile(s,kind,base,targetX,targetY,index=0){
    const b=allocBullet(s); if(!b)return; const W=weapon(s,kind);
  ...
  function hitBullet(s,b){
    ...
    const ax=b.px,ay=b.py,dx=b.x-ax,dy=b.y-ay,l2=dx*dx+dy*dy||1;
    ...
    if(best){
      const dmg=weapon(s,b.kind).damage;J.Infantry.damage(s,best,dmg,b.kind,b.x,b.y);s.stats.hits++;b.active=false;
  ...
  function explode(s,x,y,radius,source='he',big=false){
    ...
    for(const e of s.enemies){ if(e.state==='DEAD')continue; const d=Math.hypot(e.x-x,e.y-y); if(d<radius){ const fall=1-d/radius; J.Infantry.damage(s,e,weapon(s,'he').damage*(.35+.75*fall),'he',e.x,e.y); e.suppression=1; } }
    J.Vehicles.blastDamage(s,x,y,radius,weapon(s,'he').damage); J.Airborne.blastDamage(s,x,y,radius,weapon(s,'he').damage);
  ...
  function updateBullets(s,dt){
    for(const b of s.bullets){
      ...
      if(b.kind==='he'){
        const he=weapon(s,'he');
        const ah=J.Airborne.hitProjectile(s,b); if(ah&&ah.detonate){ b.active=false; explode(s,b.x,b.y,he.radius,'he',false); continue; }
        const vh=J.Vehicles.hitProjectile(s,b); if(vh&&vh.detonate){ b.active=false; explode(s,b.x,b.y,he.radius,'he',false); continue; }
        const d=Math.hypot(b.x-b.targetX,b.y-b.targetY);
        if(d<24||b.travel>=b.maxTravel||b.life<=0||b.y<0||b.x<0||b.x>s.viewport.w){ b.active=false; explode(s,b.x,b.y,he.radius,'he',false); continue; }
      }else{
  ...
  function artilleryStart(s){
    ...
    for(let i=0;i<count;i++){ const pt=J.World.randomGroundPoint(s,r,58,s.bunker.y-95); const time=C.artillery.introDelay+(i/Math.max(1,count-1))*duration+(r()-.5)*.12; s.artillery.events.push({x:pt.x,y:pt.y,time,whistle:false,done:false}); }
  }
  ...
  J.Combat={init,fireCharge,explode,update,updateArtillery,weapon};
})();
