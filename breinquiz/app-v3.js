...
function save(){try{localStorage.setItem('brainCatsV3',JSON.stringify([...enabled]));localStorage.setItem('brainCountV3',$('#count').value)}catch(e){}}
function load(){try{const c=JSON.parse(localStorage.getItem('brainCatsV3'));if(Array.isArray(c)){const ok=c.filter(x=>cats.includes(x));if(ok.length)enabled=new Set(ok)}const n=localStorage.getItem('brainCountV3');if(n)$('#count').value=n}catch(e){}}
function renderCats(){
...
function start(){
  ...
  deck=build();
  if(!deck.length){
    enabled=new Set(cats);renderCats();save();
    deck=build();
  }
  if(!deck.length)return toast('Vraagbank kon niet laden — vernieuw de pagina');
...
function reveal(){revealed=true;$('#answer').textContent=current.a;$('#explain').textContent=current.e;$('#answerBox').classList.remove('hidden');$('#reveal').classList.add('hidden');$('#next').classList.remove('hidden');$('#bar').style.width=`${(i+1)/deck.length*100}%`;setTimeout(()=>$('#answerBox').scrollIntoView({behavior:'smooth',block:'nearest'}),60)}
function next(){i++;renderQ()}
function skip(){if(revealed)return toast('Na het antwoord: ga naar volgende');if(deck.length-i<2)return toast('Laatste vraag');deck.push(deck.splice(i,1)[0]);renderQ()}
function finish(){$('#doneText').textContent=`Je hebt ${deck.length} doordenkers gehad. Binnen deze ronde is niets herhaald.`;show('done')}
