...
// Eerlijke spionselectie:
// - gebruikt crypto-random als Safari dat ondersteunt
// - bij 1 spion krijgt iedereen eerst één beurt voordat iemand terugkomt
// - dezelfde speler kan nooit direct opnieuw spion zijn
let spyBagNames = [];
let spyBagKey = '';
let previousSpyNames = [];
...
function renderPlayers(){
  ...
  players.forEach((p,i)=>{
    ...
    inp.addEventListener('input',()=>{players[i]=inp.value; resetSpyRotation(); savePrefs();});
    row.querySelector('button').addEventListener('click',()=>{
      if(players.length<=3){toast('Minimaal 3 spelers');return;}
      players.splice(i,1); resetSpyRotation(); renderPlayers(); savePrefs();
    });
    c.appendChild(row);
  });
...
function resetSpyRotation(){ spyBagNames=[]; spyBagKey=''; previousSpyNames=[]; }

function chooseSpyIndices(spyCount){
  const key=players.join('\u0001');
  const indices=players.map((_,i)=>i);

  if(spyCount===1){
    if(spyBagKey!==key || spyBagNames.length===0){
      let bag=shuffled(players);
      if(previousSpyNames.length && bag.length>1 && bag[0]===previousSpyNames[0]){
        const j=bag.findIndex(n=>n!==previousSpyNames[0]);
        if(j>0) [bag[0],bag[j]]=[bag[j],bag[0]];
      }
      spyBagNames=bag;
      spyBagKey=key;
    }
    const chosenName=spyBagNames.shift();
    previousSpyNames=[chosenName];
    return new Set([players.indexOf(chosenName)]);
  }

  // Bij 2 spionnen: met minimaal 6 spelers kiezen we eerst spelers
  // die de vorige ronde géén spion waren. Zo zijn er geen directe herhalingen.
  let eligible=indices.filter(i=>!previousSpyNames.includes(players[i]));
  if(eligible.length<spyCount) eligible=indices;
  const chosen=shuffled(eligible).slice(0,spyCount);
  previousSpyNames=chosen.map(i=>players[i]);
  spyBagNames=[];
  spyBagKey=key;
...
function startGame(){
  ...
  const pool=getPool();
  const loc=pool[randInt(pool.length)];
  const order=shuffled(players.map((name,i)=>({name,idx:i})));
  const spyIndices=chooseSpyIndices(spyCount);
...
function endRound(){
  ...
  const spies=game.assignments.filter(a=>a.spy).map(a=>a.name);
  $('#resultLocation').textContent=game.location;
  $('#resultSpies').textContent=spies.join(' & ');
  $('#roleSummary').innerHTML=game.assignments.map(a=>`<div class="locrow"><span><b>${escapeHtml(a.name)}</b></span><span>${a.spy?'🕵️ Spion':escapeHtml(a.role)}</span></div>`).join('');
  showScreen('resultScreen');
}
function resetAll(){
  if(confirm('Spelers en instellingen terugzetten naar standaard?')){
    try{localStorage.removeItem('spyfallPlayers');localStorage.removeItem('spyfallSet');}catch(e){}
    players=['Eline','Maarten','Juan','Daniëlle']; selectedSet='nl'; resetSpyRotation(); renderPlayers(); renderPills(); showScreen('setupScreen'); toast('Teruggezet');
  }
}
function addPlayer(){
  if(players.length>=20){toast('Maximaal 20 spelers');return;}
  players.push(`Speler ${players.length+1}`); resetSpyRotation(); renderPlayers(); savePrefs();
